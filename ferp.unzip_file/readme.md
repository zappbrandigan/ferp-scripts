...
Extracts a highlighted `.zip` file into a sibling directory that matches the
archive name.
...
- Operates on the `highlighted file`; the file must end with `.zip`.
- The output directory lives next to the archive.
- Warns before overwriting an existing directory with the same name.

## Usage

1. Highlight a `.zip` file in FERP.
2. Run `Unzip File` from the Scripts panel.
3. Confirm/cancel overwrites if prompted.
3. The script creates `<archive_name>/` next to the original zip and extracts contents there.
